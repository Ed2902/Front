...
const InventarioProveedor = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const conProveedor = data.filter(item => item.LoteProducto?.Proveedor)
        const agrupado = agruparPorProveedorProducto(conProveedor)
        setDataOriginal(agrupado)
      } catch (error) {
        console.error('Error cargando inventario por proveedor:', error.message)
      } finally {
        setLoading(false)
      }
    }
  ...
  const agruparPorProveedorProducto = data => {
    const mapa = {}

    for (const item of data) {
      const key = `${item.LoteProducto.Proveedor.Nombre}|${item.Producto.Id_producto}`
      if (!mapa[key]) {
        mapa[key] = {
          proveedor: item.LoteProducto.Proveedor.Nombre,
  ...
  const exportToExcel = () => {
    ...
    utils.book_append_sheet(workbook, worksheet, 'Inventario por Proveedor')
    writeFile(workbook, 'InventarioPorProveedor.xlsx')
  }
