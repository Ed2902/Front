...
const normalizeRutaRelativa = input => {
  ...
  // Si viene URL completa, extrae desde /uploads/ o desde /entradas/
  const lower = s.toLowerCase()

  // Caso 1: trae "/uploads/..."
  const idxUploads = lower.indexOf('/uploads/')
  if (idxUploads >= 0) {
    s = s.slice(idxUploads + 1) // quita el "/" inicial
  } else {
    // Caso 2: trae "/entradas/..." (tu caso)
    const idxEntradas = lower.indexOf('/entradas/')
    if (idxEntradas >= 0) {
      s = s.slice(idxEntradas + 1) // deja "entradas/..."
    }
  }
  ...
  // Si quedó "uploads/uploads/..." lo normaliza
  s = s.replace(/^uploads\/+/i, 'uploads/')

  // ✅ Si NO empieza por uploads/, lo asumimos como ruta dentro de uploads
  // Ej: "entradas/26/fotos/..." => "uploads/entradas/26/fotos/..."
  if (!/^uploads\//i.test(s)) {
    s = `uploads/${s}`
  }
