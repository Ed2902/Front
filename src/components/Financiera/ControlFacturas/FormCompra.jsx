...
import { crearFactCompra } from './FactCompra.service'
...
export default function FormCompra({ token, onCancel, onCreated }) {
  ...
  // campos (obligatorios: proveedor, factura, fecha, vencimiento)
  ...
  const errors = useMemo(() => {
    const e = {}
    if (!proveedor.trim()) e.proveedor = 'Proveedor es obligatorio'
    ...
    return e
  }, [proveedor, factura, fecha, fechaVenc])
  ...
  const onSubmit = async e => {
    ...
    try {
      ...
      await crearFactCompra(fd, token)

      setOkMsg('Compra creada correctamente ✅')
      onCreated?.()
    } catch (err) {
      setError(err?.message || 'Error creando la compra')
    } finally {
      setSaving(false)
    }
  }
      ...
      <Row className='g-2 mb-2'>
        <Col lg={4}>
          ...
          <Form.Control
            ...
            onBlur={() => touch('proveedor')}
            isInvalid={touched.proveedor && !!errors.proveedor}
            placeholder='Ej: greem'
          />
