...
export default function FactCompraTable() {
  ...
  // ✅ filtros (si cualquiera está activo: backend devuelve TODO, sin paginación)
  const [estadoFiltro, setEstadoFiltro] = useState('')
  const [lineaFiltro, setLineaFiltro] = useState('')

  // table state
  // ✅ por defecto orden por inserción (createdAt desc)
  ...
  const onEliminar = useCallback(
    async factura => {
      ...
      try {
        await eliminarFactCompra(factura._id, token)
  ...
  const cargar = async () => {
    try {
      ...
      const res = await listarFactCompra(
        {
          page,
          limit: LIMIT,
      ...
      // ✅ respetar orden backend (inserción)
      setData(mapped)

      const filtrosActivos = !!estadoFiltro || !!lineaFiltro

      // ✅ Si hay filtro: backend trae TODO => paginación se “apaga”
      if (filtrosActivos) {
        setTotalPages(1)
        setPage(1)
      } else {
  ...
  // ✅ Si cambia cualquier filtro, vuelve a página 1
  useEffect(() => {
    setPage(1)
  }, [estadoFiltro, lineaFiltro])

  useEffect(() => {
    cargar()
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [token, page, authLoading, estadoFiltro, lineaFiltro])
  ...
  const commitCell = async (rowIndex, columnId, value) => {
    ...
    if (columnId.startsWith('extra:')) {
      ...
      await actualizarFactCompra(row._id, fd, token)
      return
    }
  ...
  // ===========================
  // ✅ Scroll horizontal externo (sticky) sincronizado
  // ===========================
  ...
  useEffect(() => {
    ...
    const syncSizes = () => {
      const needs = sc.scrollWidth > sc.clientWidth + 1
      setShowHScroll(needs)
    ...
    sc.addEventListener('scroll', onScrollTable, { passive: true })
    bar.addEventListener('scroll', onScrollBar, { passive: true })
    ...
    return () => {
      sc.removeEventListener('scroll', onScrollTable)
      bar.removeEventListener('scroll', onScrollBar)
      window.removeEventListener('resize', syncSizes)
      ro.disconnect()
    }
  }, [columnOrder, columnVisibility, data])

  const filtroActivo = !!estadoFiltro || !!lineaFiltro
    ...
    <div className='excel-wrap'>
      <div className='excel-topbar'>
        ...
        <div className='excel-actions'>
          {/* ✅ filtros estado + linea */}
          <FiltroFacturas
            estadoValue={estadoFiltro}
            lineaValue={lineaFiltro}
            onEstadoChange={setEstadoFiltro}
            onLineaChange={setLineaFiltro}
            onClear={() => {
              setEstadoFiltro('')
              setLineaFiltro('')
            }}
      ...
      <div className='excel-scrollbox'>
        {showHScroll && (
          <div className='excel-hscroll' ref={barRef} aria-hidden='true'>
            <div className='excel-hscroll-inner' ref={barInnerRef} />
          </div>
        )}
      ...
      {/* ✅ Paginación: solo cuando NO hay filtros */}
      <div className='excel-footer'>
        ...
        <div className='excel-footer-right'>
          <Button
            ...
            disabled={filtroActivo || page <= 1 || loading}
          >
            ← Anterior
          </Button>
          <Button
            ...
            disabled={filtroActivo || page >= totalPages || loading}
          >
            Siguiente →
          </Button>
        </div>
